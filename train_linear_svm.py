...
def main():
    ...
    tfidf = TfidfVectorizer(max_features=5000, ngram_range=(1, 1), min_df=10)
    ...
    evaluate_model("Simple TF-IDF + Linear SVM (Val)", y_val, y_val_pred, label_names)

    print("\ntest results:")
    y_test_pred = model.predict(X_test_tfidf)
    result = evaluate_model("Simple TF-IDF + Linear SVM (Test)", y_test, y_test_pred, label_names)

    pd.DataFrame([result]).to_csv(r"dataset\simple_svm_results.csv", index=False)
    print("\nSaved results to dataset\\simple_svm_results.csv")
